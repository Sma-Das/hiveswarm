...
export function ReportView({ report, loading, apiUrl, projectId }: { report: ReportData | null; loading: boolean; apiUrl: string; projectId: string }) {
  ...
  return (
    <section className="management-view report-view" aria-labelledby="report-title">
      <div className="management-heading">
        <div><p className="eyebrow">Live evaluation output</p><h1 id="report-title">Security report</h1><p>Generated {new Date(report.generatedAt).toLocaleString()} from the current evidence graph.</p></div>
        <a className="button button--primary" href={`${apiUrl}/api/reports/current?format=markdown&download=1&projectId=${encodeURIComponent(projectId)}`}><Download size={16} aria-hidden="true" />Download Markdown</a>
      </div>
